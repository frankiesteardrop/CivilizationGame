...
public class TribeController implements GameEventListener {
    ...
    public void spawnInitialTribes() {
        List<Hex> candidates = map.getHexes().stream()
                ...
                .filter(h -> map.getHexDistance(map.getTownHall().getQ(), map.getTownHall().getR(), h.getQ(), h.getR()) >= MIN_FROM_TH_DISTANCE)
    ...
    private Hex findBestHexForTribe(TribeType type, List<Hex> candidates, List<Hex> occupied) {
        TerrainType preferred = switch (type) {
            ...
            case WARRIOR, COMMERCIAL, COASTAL -> TerrainType.PLAINS;
            case MOUNTAIN -> TerrainType.MOUNTAIN;
        };

        for (Hex h : candidates) if (h.getTerrainType() == preferred && isFarEnough(h, occupied)) return h;
        for (Hex h : candidates) if (isFarEnough(h, occupied)) return h;
        return null;
    }
    ...
    public void processTribesTurn() {
        ...
        for (Hex hex : map.getHexes()) {
            ...
            // تزریق دیزاین پترن: استیت قبیله تصمیم می‌گیرد چه واکنشی نشان دهد
    ...
    public boolean requestPeace(Tribe tribe) {
        ...
        tribe.addRelationship(90); // به حالت خنثی یا ناراضی برمی‌گرداند
        if (tribe.getRelationship() > -10) {
            tribe.addRelationship(-(tribe.getRelationship() + 10)); // تنظیم روی -10
        }
    ...
    public void onUnitKilled(Unit unit) {
        // واگذاری بررسی پیشرفت مأموریت به MissionState
        for (Hex h : map.getHexes()) {
