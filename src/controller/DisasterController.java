...
public class DisasterController {
    ...
    private final List<DisasterStrategy> disasterStrategies;
    ...
    public void checkAndTriggerDisasters() {
        ...
        List<DisasterStrategy> available = disasterStrategies.stream()
                .filter(d -> d.canTrigger(map, currentSeason))
                .collect(Collectors.toList());

        if (!available.isEmpty()) {
            DisasterStrategy chosen = available.get(random.nextInt(available.size()));
    ...
    private static class EarthquakeStrategy implements DisasterStrategy {
        @Override
        public boolean canTrigger(GameMap map, Season currentSeason) {
            return map.getHexes().stream().anyMatch(h -> h.getTerrainType() != TerrainType.SEA);
        }

        @Override
        public void trigger(GameMap map, Random random) {
    ...
    private static class FloodStrategy implements DisasterStrategy {
        @Override
        public boolean canTrigger(GameMap map, Season currentSeason) {
        ...
        public void trigger(GameMap map, Random random) {
            ...
            int roadsDestroyed = 0, farmsDestroyed = 0;
            ...
            if (roadsDestroyed > 0 || farmsDestroyed > 0) {
                GameEventDispatcher.fireNotification(String.format("🌊 Flood destroyed %d road(s) and %d farm(s)!", roadsDestroyed, farmsDestroyed));
            }
        }
    }

    private static class BearSpawnStrategy implements DisasterStrategy {
        @Override
        public boolean canTrigger(GameMap map, Season currentSeason) {
            long currentBears = map.getUnits().stream()
                    .filter(u -> u.isAlive() && u.getType() == UnitType.BEAR).count();
            return map.getBearCooldown() <= 0 && currentBears < BEAR_MAX_COUNT;
        }
        ...
        public void trigger(GameMap map, Random random) {
            ...
            long currentBears = map.getUnits().stream()
                    .filter(u -> u.isAlive() && u.getType() == UnitType.BEAR).count();
            int newBears = (int) Math.min(1 + random.nextInt(2), BEAR_MAX_COUNT - currentBears);

            if (newBears <= 0) return;

            for (int i = 0; i < newBears; i++) {
                map.addUnit(UnitFactory.createUnit(UnitType.BEAR, forestHex.getQ(), forestHex.getR()));
            }
            ...
            List<Hex> affectedHexes = map.getHexes().stream()
                    .filter(h -> h.getTerrainType() == TerrainType.FOREST
                            && map.getHexDistance(forestHex.getQ(), forestHex.getR(), h.getQ(), h.getR()) <= 3)
                    .collect(Collectors.toList());
            ...
            if (!forestHex.isVisible()) {
                GameEventDispatcher.fireNotification("⚠️ Wild Bears have emerged from a distant forest!");
            }
        }
    }
    ...
    private void processSingleBearAI(Unit bear) {
        ...
        if (target == null) { retreatBear(bear, lairQ, lairR); return; }

        int distToTarget = map.getHexDistance(bear.getQ(), bear.getR(), target.getQ(), target.getR());

        if (distToTarget > 1 && bear.getCurrentAP() >= 1) {
            moveBearTowardTarget(bear, target, lairQ, lairR);
            distToTarget = map.getHexDistance(bear.getQ(), bear.getR(), target.getQ(), target.getR());
        }

        if (distToTarget <= 1 && bear.getCurrentAP() >= 1) {
            bear.consumeAP(1);
            boolean isMilitaryTarget = (target.getType() == UnitType.SWORDSMAN || target.getType() == UnitType.ARCHER || target.getType() == UnitType.CAVALRY);
            ...
            if (isMilitaryTarget) {
                ...
                if (bearDie > maxDefDie) {
                    ...
                    if (!target.isAlive()) GameEventDispatcher.fireNotification("🐻 A bear overpowered your military unit! (Dice: " + bearDie + " vs " + maxDefDie + ")");
                    else GameEventDispatcher.fireNotification("🐻 Bear attack! Military unit took " + dmg + " damage. (Dice: " + bearDie + " vs " + maxDefDie + ")");
                } else {
                    bear.kill();
    ...
    private Unit findBearTarget(Unit bear, int lairQ, int lairR) {
        Optional<Unit> civilian = map.getUnits().stream()
                .filter(u -> u.isAlive() && (u.getType() == UnitType.WORKER || u.getType() == UnitType.BUILDER || u.getType() == UnitType.EXPLORER || u.getType() == UnitType.BORDER_EXPANDER) && map.getHexDistance(lairQ, lairR, u.getQ(), u.getR()) <= BEAR_DETECT_RADIUS)
    ...
    private Hex findNearestForest(int q, int r) {
        return map.getHexes().stream()
                .filter(h -> h.getTerrainType() == TerrainType.FOREST)
                .min(Comparator.comparingInt(h -> map.getHexDistance(q, r, h.getQ(), h.getR())))
                .orElse(null);
    }
}